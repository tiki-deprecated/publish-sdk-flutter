...
class TikiSdkFlutterPlatform extends PlatformInterface {
  ...
  Future<void> methodHandler(MethodCall call) async {
    switch (call.method) {
      ...
      case "assignOwnership" :
        ...
        TikiSdkDataTypeEnum type = TikiSdkDataTypeEnum.fromValue(call.arguments['type']);
      ...
      case "modifyConsent" :
        ...
        TikiSdkDestination destination = TikiSdkDestination.fromJson(call.arguments['destination']);
        String? about = call.arguments['about'];
        String? reward = call.arguments['reward'];
        DateTime? expiry = DateTime.fromMillisecondsSinceEpoch(call.arguments['expiry'] ~/ 1000);
        await tikiSdk.modifyConsent(ownershipId, destination, about: about, reward: reward, expiry: expiry);
        break;
      case "applyConsent" :
        String source = call.arguments['source'];
        TikiSdkDestination destination = TikiSdkDestination.fromJson(call.arguments['destination']);
  ...
  static Future<Map<String, Uint8List>> getAll(String address) async {
    Map<String, Uint8List>? response = await instance.methodChannel.invokeMapMethod<String, Uint8List>(
  ...
  static Future<Uint8List?> read(String path) async =>
      await instance.methodChannel.invokeMethod<Uint8List>(
        'read', {'path': path});

  static Future<void> write(String path, Uint8List obj) async =>
    await instance.methodChannel.invokeMethod('write', {'path': path, 'obj': obj});

  static Future<void> callRequest(String requestId) async =>
      await instance.methodChannel.invokeMethod('callRequest', {'requestId': requestId});


  static void blockRequest(String requestId, String val) async =>
      await instance.methodChannel.invokeMethod('blockRequest', {'requestId': requestId, 'val': val});
